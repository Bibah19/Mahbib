...
/** Builds the styled HTML body for the cancellation notice, mirroring the invite card layout but using release text addressed to the guest by name. */
function buildCancellationHtml(reservation: Reservation): string {
  return `
    <div style="font-family:Georgia,'Times New Roman',serif;background:#f7f9f8;padding:32px;color:#0f1b2d">
      <div style="max-width:560px;margin:auto;background:#ffffff;border:1px solid rgba(22,55,95,.22);border-radius:24px;padding:32px">
        <p style="font-family:Arial,sans-serif;letter-spacing:.24em;text-transform:uppercase;font-size:11px;color:#0a6b51;font-weight:800;margin:0 0 18px">
        ...
        <p style="color:#55667a;margin:0 0 22px">
          Dear ${reservation.name}, this is a short note from the couple. Your seat on ${reservation.table}, Seat ${reservation.seat}, has been released by the host.
        </p>
        <table style="font-family:Arial,sans-serif;font-size:14px;border-collapse:collapse;width:100%">
          <tr><td style="padding:8px 0;color:#55667a">Table</td><td style="padding:8px 0;font-weight:700">${reservation.table}</td></tr>
          <tr><td style="padding:8px 0;color:#55667a">Seat number</td><td style="padding:8px 0;font-weight:700">${reservation.seat}</td></tr>
        ...
        <p style="font-family:Arial,sans-serif;font-size:14px;color:#55667a;margin-top:22px">
...
/** Sends (or intentionally skips) the seat-release cancellation email. Never throws. */
  ...
  try {
    const response = await fetch(RESEND_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        to: [reservation.email],
        reply_to: WEDDING_DETAILS.contactEmail,
        subject: buildCancellationSubject(reservation),
        text: buildCancellationBody(reservation),
        html: buildCancellationHtml(reservation),
      }),
    });

    if (!response.ok) {
      const detail = await response.text();
      console.error("Resend rejected the cancellation email:", response.status, detail);
      return { status: "failed", message: "The cancellation email could not be sent." };
    }

    return { status: "sent", message: "Cancellation email sent." };
  } catch (error) {
